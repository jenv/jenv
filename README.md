...
3. Add `jenv init` to your shell to enable shims and autocompletion.
    ...
    **Fish note**: 
    Instead, copy `~/.jenv/fish/jenv.fish` to `~/.config/fish/function/jenv.fish`. If you don't have the `export` function, also copy `export.fish`
    ~~~ sh
        cp ~/.jenv/fish/jenv.fish ~/.config/fish/function/jenv.fish
        cp ~/.jenv/fish/export.fish ~/.config/fish/function/export.fish
    ~~~
    **Fish note**:
    Modify your `~/.config/fish/config.fish` to append
    ~~~
        eval jenv init
    ~~~
